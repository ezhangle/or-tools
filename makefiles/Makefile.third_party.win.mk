...
dependencies\sources\Cbc-$(CBC_TAG)\configure:
	tools\wget -P dependencies\archives --no-check-certificate ${CBC_ARCHIVE} || (@echo wget failed to dowload $(CBC_ARCHIVE), try running 'tools\wget -P dependencies\archives --no-check-certificate $(CBC_ARCHIVE)' then rerun 'make third_party' && exit 1)
	tools\tar xzvf dependencies/archives/Cbc-${CBC_TAG}.tgz -C dependencies/sources/
